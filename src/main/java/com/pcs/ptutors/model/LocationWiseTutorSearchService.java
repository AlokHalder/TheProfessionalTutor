 ...
 * @author ALOK
 */
public class LocationWiseTutorSearchService {
        ...
        if (null != tutorRegistration) {
            ...
            try {
                ...
                String locationId = tutorRegistration.getTutorCityLocationId();
                 if (null != locationId && !"".equalsIgnoreCase(locationId)) {

                    String sqlQuery = "select td.tutor_id from  tutor_details td where td.location_id ="+locationId;
    ...
    public TutorRegistrationVO populateTutorProfileByTutorId(String tutorId, Connection conn, PreparedStatement pstmt, ResultSet rst) throws Exception, ProfessionalTutorsException {
        ...
        if (null != tutorId) {
            try {          
                ...
                StringBuffer sqlQueryPart = new StringBuffer("select td.tutor_id ,td.name ,td.age,td.gender,td.address ,td.contact_number ,cn.country_name,loc.location_name,td.email_address ,td.years_of_tutoring_experience,td.preferred_tutoring_area,td.expected_fees,td.tutoring_experience,td.registration_date,td.image_flag,td.membership_category from tutor_details td , country_name cn , location_name loc where cn.country_code = loc.country_code and cn.country_code = td.country_code and loc.country_code = td.country_code and loc.location_id = td.location_id and td.tutor_id=?");
